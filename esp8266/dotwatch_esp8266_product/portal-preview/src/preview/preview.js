import { mockNetworks, mockStatus } from "../mocks/mock-device.js";
...
function openDashboardPage(pageId, updateHash = true) {
  ...
  setText("portalPageTitle", nav?.dataset.pageTitle || target.dataset.pageTitle || "ESP8266 Dashboard");
  ...
  try {
    if (state.useMock) {
      ...
      if (data.localAdminProtected && !data.deviceCode) {
        ...
        setConnection("is-online", "เชื่อม ESP8266 แล้ว");
      }
    }

    if (notify) showToast("อัปเดตสถานะแล้ว");
  } catch (error) {
    setConnection("is-offline", error.status === 403 ? "PIN ไม่ถูกต้อง" : "เชื่อม ESP8266 ไม่ได้");
    if (!state.lastStatus) applyStatus(mockStatus);
    if (notify) showToast(error.message || "ไม่สามารถอ่านสถานะจาก ESP8266");
  } finally {
    state.polling = false;
  }
}
...
async function scanWifi() {
  ...
  setText("scanStatus", "กำลังสแกน Wi-Fi รอบตัว ESP8266...");
...
function handlePreviewAction(action) {
  switch (action) {
    ...
    case "json":
      ...
      break;
    case "sensor":
      showToast("Preview mode: การอ่าน Sensor จริงยังทำผ่านหน้า ESP8266");
      break;
    case "restart":
...
function bindEvents() {
  ...
  byId("previewMockToggle")?.addEventListener("click", (event) => {
    ...
    event.currentTarget.textContent = state.useMock ? "ใช้ ESP8266 จริง" : "ใช้ Mock Data";
    refreshStatus({ notify: true });
    scanWifi();
  });
...
function init() {
  const configuredTarget = document.querySelector('meta[name="esp8266-target"]')?.content;
  ...
  if (mockToggle) {
    ...
    mockToggle.textContent = state.useMock ? "ใช้ ESP8266 จริง" : "ใช้ Mock Data";
  }
