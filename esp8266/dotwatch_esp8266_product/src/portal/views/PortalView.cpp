...
String PortalView::dashboardPage() {
  ...
  html += "<title>dotTH ESP8266</title>";
  html += "<link rel='stylesheet' href='/portal.css'></head><body>";
  ...
  html += overviewPageHtml();
  html += wifiPageHtml();
  html += devicePageHtml();
  html += sensorPageHtml();
  html += securityPageHtml();
  html += systemPageHtml();
  html += "</main><footer class='portal-footer'>";
  html += DOTWATCH_MODEL_NAME;
  html += " · Modular Product Core · Hold FLASH 6 seconds for Factory Reset</footer></div></div>";
  html += "<script src='/portal.js'></script></body></html>";
  return html;
}
...
String PortalView::loginPage(const String &message,
  ...
  String body;
  body += "<section class='card login-card'><div class='login-logo'>⚙</div><div class='kicker'>Protected setup</div><h1>เข้าสู่ ESP8266 Device Console</h1>";
...
String PortalView::pageShell(const String &title,
                             const String &body) {
  String html;
  html.reserve(body.length() + 5200);
  html += "<!doctype html><html lang='th'><head><meta charset='utf-8'>";
  html += "<meta name='viewport' content='width=device-width,initial-scale=1,viewport-fit=cover'>";
  html += "<meta name='theme-color' content='#020617'>";
  html += "<title>" + StringUtils::htmlEscape(title) + "</title>";
  html += "<link rel='stylesheet' href='/portal.css'></head><body>";

  if (title == "ESP8266 Local Admin") {
    html += "<div class='login-shell'><aside class='login-brand-panel'><div class='login-brand'><div class='brand-mark'>dT</div><div><h1>dotTH</h1></div></div><div class='login-brand-copy'><div class='kicker'>Local Device Management</div><h2>จัดการ ESP8266 ในรูปแบบเดียวกับ Dashboard</h2><p>ใช้รายละเอียดและระบบเดิมของ Firmware แต่จัดโครงสร้างใหม่ให้อ่านง่าย แยกเมนูชัดเจน และเหมาะกับการใช้งานจริง</p></div><div class='help'>";
    html += DOTWATCH_MODEL_NAME;
