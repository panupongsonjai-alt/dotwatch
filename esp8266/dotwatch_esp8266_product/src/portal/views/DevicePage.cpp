...
String PortalView::devicePageHtml() {
  String html;
  html += "<section id='device' class='dashboard-page' data-page-title='Device Connection'><div class='card'><div class='section-head'><div><div class='kicker'>Device Settings</div><h2>เชื่อม dotTH Backend</h2><p class='muted'>แก้เฉพาะเมื่อเปลี่ยน Backend หรือผูก ESP8266 กับ Device ใหม่</p></div><span id='deviceBadge' class='badge ";
  ...
  html += "<div class='field'><label>Device Code</label><input id='deviceCode' name='deviceCode' value='" + StringUtils::htmlEscape(config_->deviceCode) + "' placeholder='DW-ESP8266-...'></div>";
