...
#define FIRMWARE_VERSION "esp32-dht3-security-0.7.0"
...
String tlsModeText() {
  if (cfg.tlsCaCert.length() > 0) {
    return "Root CA enabled";
  }
...
void handleRoot() {
  ...
  body += "<label class='full'>Root CA Certificate (optional)<textarea name='tlsCaCert' placeholder='วาง Root CA PEM ที่นี่เพื่อเปิด TLS verification · เว้นว่างเพื่อใช้ค่าเดิม · พิมพ์ CLEAR เพื่อลบ Root CA'></textarea></label>";
  body += "<div class='notice full'>TLS Mode ปัจจุบัน: " + htmlEscape(tlsModeText()) + " · Production default ต้องใส่ Root CA ก่อนส่ง HTTPS · เปิด insecure fallback ได้เฉพาะตอนทดสอบด้วย build flag เท่านั้น</div>";
...
bool postIngest(float temperature, float humidity, int rssi) {
  ...
  if (endpoint.startsWith("https://")) {
    if (hasTlsCaCert()) {
      secureClient.setCACert(cfg.tlsCaCert.c_str());
      Serial.println("TLS mode: Root CA verification enabled");
    } else {
#if DOTWATCH_ALLOW_INSECURE_TLS_FALLBACK
