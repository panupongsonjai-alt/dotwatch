...
def flush_offline_queue(queue):
    ...
    if getattr(settings, "queue_flush_batch_enabled", False) and settings.queue_flush_limit > 1:
...
def main():
    ...
    while RUNNING:
        ...
        try:
            ...
            # If the agent sent or attempted to flush offline queue data in this cycle,
            # pause here. Sending a live payload immediately after a queued payload can
            # trigger backend per-device throttling (HTTP 429).
            if flush_result["sent"] or flush_result["error"]:
                sleep_interruptible(settings.send_interval_seconds)
                continue
