...
import {
  ...
  normalizeAdminOverviewPreferences,
...
function AdminOverview({ stats, users, devices, loading }) {
  ...
  const showRecordLists =
    displayPreferences.showRecentUsers ||
    displayPreferences.showLatestDevices

  useEffect(() => {
    function syncPreferences(event) {
      setDisplayPreferences(
        event.detail?.overview
          ? normalizeAdminOverviewPreferences(event.detail.overview)
          : readAdminOverviewPreferences()
      )
    }

    function syncStorage() {
      setDisplayPreferences(readAdminOverviewPreferences())
    }

    window.addEventListener(ADMIN_SETTINGS_EVENT, syncPreferences)
    window.addEventListener('storage', syncStorage)

    return () => {
      window.removeEventListener(ADMIN_SETTINGS_EVENT, syncPreferences)
            ...
            {displayPreferences.showRecentUsers && (
              <article className="admin-panel admin-overview-record-panel">
                ...
                <div className="admin-overview-record-list" role="list">
                  <div
                    className="admin-overview-record-columns"
                    aria-hidden="true"
                  >
                    <span>User</span>
                    <span>Plan</span>
                  ...
                  {recentUsers.length ? (
                    recentUsers.map((user) => (
                      ...
                      >
                        <div className="admin-overview-record-primary">
                          <strong>
                            {getValue(user, ['name'], 'Unnamed user')}
                          </strong>
                        ...
                        <span
                          ...
                          title={
                            user.databaseUsagePending
                              ...
                              : `Calculated ${getValue(user, [
                                  'databaseUsageCalculatedAt',
                                ])}`
                          }
                        >
                          {user.databaseUsagePending
                    ...
                    <div className="admin-overview-record-empty">
                      No users found.
                    </div>
                  )}
                </div>
            ...
            {displayPreferences.showLatestDevices && (
              <article className="admin-panel admin-overview-record-panel">
                <div className="panel-header admin-overview-record-header">
                  ...
                  <span>
                    Showing {latestDevices.length} of{' '}
                ...
                <div className="admin-overview-record-list" role="list">
                  <div
                    className="admin-overview-record-columns admin-overview-device-columns"
                    aria-hidden="true"
                  >
                    <span>Device</span>
                    <span>Owner</span>
                  ...
                  {latestDevices.length ? (
                    latestDevices.map((device) => (
                      ...
                      >
                        <div className="admin-overview-record-primary">
                          <strong>
                            {getValue(device, ['name'], 'Unnamed device')}
                          </strong>
                          <span>
                            {getValue(device, ['deviceCode', 'device_code'])}
                          </span>
                        </div>
                        <span className="admin-overview-record-value">
                          {getValue(device, [
