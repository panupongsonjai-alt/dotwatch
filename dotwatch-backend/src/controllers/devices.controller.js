...
export async function listDevices(req, res) {
  ...
  const result = await pool.query(
    `SELECT d.id, d.device_code, d.name, d.status, d.last_seen_at, d.firmware_version,
            lr.temperature, lr.humidity, lr.rssi, lr.time AS latest_time
     FROM devices d
     LEFT JOIN LATERAL (
...
export async function createDevice(req, res) {
  ...
  const deviceCode = req.body.deviceCode || `DW-${crypto.randomInt(1, 999999).toString().padStart(6, '0')}`;
  const deviceSecret = req.body.deviceSecret || crypto.randomBytes(18).toString('hex');
  ...
  res.status(201).json({ ...result.rows[0], deviceSecret });
}
