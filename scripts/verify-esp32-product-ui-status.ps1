...
Write-Host 'dotWatch Product UI Status verification' -ForegroundColor Red
Write-Host '============================================================' -ForegroundColor Red
...
$checks = [ordered]@{
    'Firmware product UI version' = $firmware -match '1\.2\.1-product-ui-status'
    'Landscape rotation configured' = $productConfig -match 'TFT_ROTATION\s*=\s*1'
    'Power sense config added' = $productConfig -match 'POWER_SENSE_PIN\s*=\s*-1'
    'Display width 320' = $displayHeader -match 'DISPLAY_WIDTH\s*=\s*320'
    'Display height 240' = $displayHeader -match 'DISPLAY_HEIGHT\s*=\s*240'
    'Product metric block builder' = $displayHeader -match 'createMetricBlock'
    'Signal uses WiFi status' = $displaySource -match 'updateSignalStrength\(bool wifiConnected, int rssi\)'
    'Battery uses power status' = $displaySource -match 'readPowerConnected\(\) const'
    'Battery indicator updates dynamically' = $displaySource -match 'updateBatteryIndicator\(bool powerConnected\)'
    'Status pill present' = $displaySource -match 'statusPill_'
    'Red product palette' = $displaySource -match 'COLOR_RED_HEX'
    'Green online palette' = $displaySource -match 'COLOR_GREEN_HEX'
    'Humidity one decimal place' = $displaySource -match '"%.1f", status\.lastHumidity'
    ...
    'Architecture banner updated' = $appSource -match 'polished product display'
}
