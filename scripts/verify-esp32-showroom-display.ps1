...
Write-Host 'dotWatch Showroom Display verification' -ForegroundColor Red
Write-Host '============================================================' -ForegroundColor Red
...
$checks = [ordered]@{
    'Firmware showroom display version' = $firmware -match '1\.2\.3-showroom-display'
    ...
    'Header created' = $displayHeader -match 'createHeader\(\)'
    'Metric cards created' = $displayHeader -match 'createMetricCard'
    'WiFi signal group created' = $displayHeader -match 'createWiFiSignalGroup'
    'Battery group created' = $displayHeader -match 'createBatteryGroup'
    'Connection card created' = $displayHeader -match 'createConnectionCard'
    'WiFi uses connection and RSSI' = $displaySource -match 'updateWiFiStatus\(bool connected, int rssi\)'
    'Power uses runtime status' = $displaySource -match 'readPowerConnected\(\) const'
    'Metric status updates from thresholds' = $displaySource -match 'DISPLAY_TEMP_NORMAL_MIN'
    ...
    'Architecture banner updated' = $appSource -match 'showroom product display'
}
