...
void sendLocalAdminLogin(int statusCode = 200, String message = "") {
  ...
  if (cfg.adminPin.length() == 0) {
    body += "<div class='notice'>à¸¢à¸±à¸‡à¹„à¸¡à¹ˆà¹„à¸”à¹‰à¸•à¸±à¹‰à¸‡ Custom PIN à¸„à¹ˆà¸²à¹€à¸£à¸´à¹ˆà¸¡à¸•à¹‰à¸™à¸„à¸·à¸­ 6 à¸•à¸±à¸§à¸—à¹‰à¸²à¸¢à¸‚à¸­à¸‡ Device Code à¸ˆà¸™à¸à¸§à¹ˆà¸²à¸ˆà¸°à¸•à¸±à¹‰à¸‡ PIN à¹ƒà¸«à¸¡à¹ˆà¹ƒà¸™à¸«à¸™à¹‰à¸²à¸™à¸µà¹‰</div>";
  }
...
bool requireAdmin() {
  ...
  sendLocalAdminLogin(403, "PIN à¹„à¸¡à¹ˆà¸–à¸¹à¸à¸•à¹‰à¸­à¸‡ à¸«à¸£à¸·à¸­à¸¢à¸±à¸‡à¹„à¸¡à¹ˆà¹„à¸”à¹‰à¸à¸£à¸­à¸ PIN");
  return false;
}
...
String pageShell(const String &title, const String &body) {
  ...
  html += "</h1><div class='muted'>dotWatch ESP32-DHT3 Â· Additional model Â· ";
  ...
  html += "</div></header>";
  html += body;
  html += "<div class='footer'>dotWatch ESP32 Config Portal Â· à¹„à¸¡à¹ˆà¹à¸—à¸™à¸—à¸µà¹ˆ Raspberry Pi / DW20CH</div>";
  html += "</div></body></html>";
  return html;
}
...
String currentStatusHtml() {
  ...
  html += "<div class='stat'><small>DHT</small><strong>Pin " + String(cfg.dhtPin) + " Â· " + String(cfg.dhtType == DHT22 ? "DHT22" : "DHT11") + "</strong></div>";
  ...
  html += "<div class='stat'><small>Last Send</small><strong>" + htmlEscape(lastSendStatus) + " Â· HTTP " + String(lastHttpStatus) + "</strong></div>";
  html += "<div class='stat'><small>Last Error</small><strong>" + htmlEscape(lastSendError.length() ? lastSendError : "None") + "</strong></div>";
  html += "<div class='stat'><small>Send Count</small><strong>OK " + String(totalSendOk) + " Â· FAIL " + String(totalSendFail) + "</strong></div>";
  return html;
}
...
void handleTest() {
  ...
  if (ok) {
    body += "<div class='stat'><small>metric_1 Temperature</small><strong>" + String(temperature, 2) + " Â°C</strong></div>";
...
void handleRoot() {
  ...
  if (!hasRequiredConfig()) {
    body += "<div class='notice'>à¸¢à¸±à¸‡à¸•à¸±à¹‰à¸‡à¸„à¹ˆà¸²à¹„à¸¡à¹ˆà¸„à¸£à¸š à¹ƒà¸«à¹‰à¸à¸£à¸­à¸ Wiâ€‘Fi, Backend, Device Code à¹à¸¥à¸° Device Secret à¸‚à¸­à¸‡ ESP32 device à¹ƒà¸«à¸¡à¹ˆ à¹„à¸¡à¹ˆà¹ƒà¸Šà¹‰à¸‚à¸­à¸‡ Raspberry Pi</div>";
  }
  ...
  body += "<label>Wiâ€‘Fi SSID<input name='wifiSsid' value='" + htmlEscape(cfg.wifiSsid) + "' placeholder='Your Wi-Fi'></label>";
  body += "<label>Wiâ€‘Fi Password<input type='password' name='wifiPassword' value='' placeholder='à¹€à¸§à¹‰à¸™à¸§à¹ˆà¸²à¸‡à¹€à¸žà¸·à¹ˆà¸­à¹ƒà¸Šà¹‰à¸„à¹ˆà¸²à¹€à¸”à¸´à¸¡'></label>";
  ...
  body += "<label>Device Secret<input type='password' name='deviceSecret' value='' placeholder='à¹€à¸§à¹‰à¸™à¸§à¹ˆà¸²à¸‡à¹€à¸žà¸·à¹ˆà¸­à¹ƒà¸Šà¹‰à¸„à¹ˆà¸²à¹€à¸”à¸´à¸¡'></label>";
  body += "<label>Local Admin PIN<input type='password' name='adminPin' value='' placeholder='à¹€à¸§à¹‰à¸™à¸§à¹ˆà¸²à¸‡à¹€à¸žà¸·à¹ˆà¸­à¹ƒà¸Šà¹‰à¸„à¹ˆà¸²à¹€à¸”à¸´à¸¡'></label>";
...
void handleSave() {
  ...
  if (next.adminPin.length() > 0 && next.adminPin.length() < 4) {
    String body = "<section class='card'><h2>Invalid PIN</h2><div class='notice'>Local Admin PIN à¸•à¹‰à¸­à¸‡à¸¡à¸µà¸­à¸¢à¹ˆà¸²à¸‡à¸™à¹‰à¸­à¸¢ 4 à¸•à¸±à¸§à¸­à¸±à¸à¸©à¸£</div><div class='row'><a class='btn btn2' href='/'>Back</a></div></section>";
  ...
  String body = "<section class='card'><h2>Saved</h2><p class='muted'>à¸šà¸±à¸™à¸—à¸¶à¸à¹à¸¥à¹‰à¸§ à¸à¸³à¸¥à¸±à¸‡ restart ESP32 à¸ à¸²à¸¢à¹ƒà¸™ 2 à¸§à¸´à¸™à¸²à¸—à¸µ</p><a class='btn btn2' href='/'>Back</a></section>";
...
void handleReset() {
  ...
  clearConfig();
  String body = "<section class='card'><h2>Config Cleared</h2><p class='muted'>à¸¥à¹‰à¸²à¸‡à¸„à¹ˆà¸²à¹à¸¥à¹‰à¸§ à¸à¸³à¸¥à¸±à¸‡ restart à¹€à¸‚à¹‰à¸²à¸ªà¸¹à¹ˆ setup AP à¹ƒà¸«à¸¡à¹ˆ</p></section>";
