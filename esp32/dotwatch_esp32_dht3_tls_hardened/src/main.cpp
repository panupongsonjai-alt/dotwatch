...
/*
  ...
  Phase 5A TLS hardening:
    ...
    - Keeps fallback setInsecure() only when Root CA is empty
*/
...
#define FIRMWARE_VERSION "esp32-dht3-tls-hardening-0.5.0"
...
void handleRoot() {
  ...
  body += "<div class='notice full'>TLS Mode ปัจจุบัน: " + htmlEscape(tlsModeText()) + " · ถ้ายังไม่ใส่ Root CA ระบบจะใช้ insecure fallback เพื่อไม่ให้ ESP32 หลุดจาก backend</div>";
...
bool postIngest(float temperature, float humidity, int rssi) {
  ...
  if (endpoint.startsWith("https://")) {
    if (hasTlsCaCert()) {
      ...
      Serial.println("TLS mode: insecure fallback (no Root CA configured)");
    }
